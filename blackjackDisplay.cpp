/* Title: Project 1 ECE373 blackjackDisplay Class C++ File
 * Author: John Shield
 ...
 */
blackjackDisplay::blackjackDisplay(void) {
 ...
 */
blackjackDisplay::~blackjackDisplay() {
 ...
 */
int blackjackDisplay::captureInput(void) {
...
/*
 * Function: Updates all the information in the blackjackDisplay class on window resize
 ...
 *		class, because your game should redraw the blackjackDisplay on a resize.
 */
void blackjackDisplay::handleResize(int sig) {
 ...
 */
void blackjackDisplay::displayCard(int x, int y, int suit, int number, int printAtt) {
 ...
 */
void blackjackDisplay::printFace(int suit, int number, int line, int printAtt) {
 ...
 */
void blackjackDisplay::printSuit(int suit) {
	switch (suit) {
		case 1:
            printw("%s",spades);
            break;
 ...
 * Description: This is just a look up table.
 */
void blackjackDisplay::printNumber(int number) {
    switch (number) {
        case 2:
        case 3:
        case 4:
 ...
 */
void blackjackDisplay::eraseBox(int x, int y, int sizeX, int sizeY) {
	string strDraw;
	int yCount;
	int maxSizeX;
 ...
 *			display settings.
 */
void blackjackDisplay::drawBox(int x, int y, int sizeX, int sizeY, int printAtt) {
	string strDraw;
    int ii;
    int yCount;
 ...
 */
void blackjackDisplay::bannerBottom(string bannerText) {
 ...
 *      of the screen. Does not handle carriage returns on the string.
 */
void blackjackDisplay::bannerTop(string bannerText) {
	// change to the banner draw settings
	attron(COLOR_PAIR(6) | A_REVERSE | A_BOLD);
	// checks if the banner string size is smaller than the width of the screen
    if((unsigned)cols > bannerText.size()) {
